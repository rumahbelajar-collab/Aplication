...
export type ProgramRecord = ProgramBelajar & { deskripsi?: string };
export type TutorRecord = Tutor & { idLogin: string; status?: string; telepon?: string; alamat?: string };
export type KasRecord = { id: string; tanggal: string; tipe: string; keterangan: string; jumlah: number; saldoBerjalan: number; referensiId?: string };
export type OtherIncomeRecord = PemasukanLain & { jenis?: string; nominal?: number };
export type ScheduleRecord = JadwalTutor & { waktu?: string; hari: string; tutorId: string; siswaId: string; programId: string };
export type AttendanceRecord = {
  id: string; tanggal: string; tutorId: string; tutorNama: string; siswaId: string; siswaNama: string;
  programId: string; programNama: string; fotoJurnal: string; keterangan?: string;
  status?: "pending" | "setuju" | "tolak" | "disetujui" | "ditolak" | "diproses";
  catatanAdmin?: string; tanggalProses?: string;
};
...
export function addSessionTransaction(
  db: Database,
  data: { tanggal: string; siswaId: string; tutorId: string; programId: string; catatan?: string }
): Database {
  const base = ensureDatabaseDefaults(db);
  const student = base.students.find(s => s.id === data.siswaId);
  const tutor = base.tutors.find(t => t.id === data.tutorId);
  const program = base.programs.find(p => p.id === data.programId);
  if (!student || !tutor || !program) throw new Error("Data siswa, tutor, atau program tidak ditemukan.");
  if (checkDuplicateSession(base, data)) throw new Error("Sesi untuk tutor, siswa, program, dan tanggal tersebut sudah ada.");
  const nextDb = ensureDatabaseDefaults(JSON.parse(JSON.stringify(base)));
  const newId = generateUniqueId("RP");
  nextDb.sessions = [{ id: newId, tanggal: data.tanggal, siswaId: student.id, siswaNama: student.nama, tutorId: tutor.id, tutorNama: tutor.nama, programId: program.id, programNama: program.nama, tarifSiswaSnapshot: Number(program.tarifSiswa) || 0, honorTutorSnapshot: Number(program.honorTutor) || 0, catatan: data.catatan || `Sesi pembelajaran ${program.nama}` }, ...nextDb.sessions];
  if (!student.programId) nextDb.students = nextDb.students.map(s => s.id === student.id ? { ...s, programId: program.id } : s);
  nextDb.studentLedger.push({ id: generateUniqueId("TXS"), tanggal: data.tanggal, siswaId: student.id, tipe: "debit", keterangan: `Riwayat Pertemuan [${newId}] - ${program.nama}`, jumlah: Number(program.tarifSiswa) || 0, saldoBerjalan: 0, referensiId: newId });
  nextDb.tutorLedger.push({ id: generateUniqueId("TXT"), tanggal: data.tanggal, tutorId: tutor.id, tipe: "kredit", keterangan: `Riwayat Pertemuan [${newId}] - Siswa: ${student.nama} - ${program.nama}`, jumlah: Number(program.honorTutor) || 0, saldoBerjalan: 0, referensiId: newId });
  nextDb.lastUpdated = new Date().toISOString();
  return recalculateAllLedgers(nextDb);
}
...
export function getStudentBalance(db: Database, studentId: string): number {
  return (db.studentLedger || []).filter(tx => tx.siswaId === studentId).reduce((sum, tx) => sum + (tx.tipe === "debit" ? Number(tx.jumlah) || 0 : -(Number(tx.jumlah) || 0)), 0);
}

export function getTutorHonorBalance(db: Database, tutorId: string): number {
  return (db.tutorLedger || []).filter(tx => tx.tutorId === tutorId).reduce((sum, tx) => sum + (tx.tipe === "kredit" ? Number(tx.jumlah) || 0 : -(Number(tx.jumlah) || 0)), 0);
}

export function getTutorDepositBalance(db: Database, tutorId: string): number {
  return (db.payments || []).filter(p => p.metode === "tutor" && p.tutorId === tutorId && p.statusTitipan === "pending").reduce((sum, p) => sum + (Number(p.jumlah) || 0), 0);
}

export function getKasLembagaBalance(db: Database): number {
  return (db.kas || []).reduce((sum, tx) => sum + (tx.tipe === "masuk" ? Number(tx.jumlah) || 0 : -(Number(tx.jumlah) || 0)), 0);
}
...
export function addOtherIncomeTransaction(
  ...
  const newIncome: OtherIncomeRecord = {
    id: newId, tanggal: data.tanggal, sumber: data.jenis,
    jumlah: Number(data.nominal) || 0, jenis: data.jenis, nominal: Number(data.nominal) || 0,
...
export function submitAttendanceReport(
  ...
  const newReport: AttendanceRecord = {
    id: newId,
    tanggal: data.tanggal,
    tutorId: tutor.id,
    tutorNama: tutor.nama,
...
export function verifyAttendanceReport(
  ...
  const updatedReport: AttendanceRecord = {
    ...report,
...
export function checkDuplicateSession(
  db: Database,
  data: { tanggal: string; siswaId: string; tutorId: string; programId?: string }
): boolean {
  return (db.sessions || []).some(s => s.tanggal === data.tanggal && s.siswaId === data.siswaId && s.tutorId === data.tutorId && (!data.programId || s.programId === data.programId));
}


/* =========================================================
   DATABASE MERGE / LEDGER REBUILD / DELETE HELPERS
   ========================================================= */
...
function mergeSessions(local: RiwayatPertemuan[], remote: RiwayatPertemuan[]): RiwayatPertemuan[] {
  const result = mergeArrayById(local, remote);
  const seen = new Set<string>();
  return result.filter(item => {
    const key = `${item.tanggal}|${item.tutorId}|${item.siswaId}|${item.programId}`;
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  });
}
function mergeAttendance(local: any[], remote: any[]): any[] {
  const map = new Map<string, any>();
  for (const item of local) if (item?.id) map.set(item.id, item);
  for (const item of remote) if (item?.id) map.set(item.id, { ...(map.get(item.id) || {}), ...item });
  const result: any[] = Array.from(map.values());
  const seen = new Set<string>();
  return result.filter(item => {
    const key = `${item.tanggal}|${item.tutorId}|${item.siswaId}|${item.programId}`;
    if (item.status === "pending" && seen.has(key)) return false;
    if (item.status === "pending") seen.add(key);
    return true;
  });
}
export function mergeDatabases(localInput: Database | null | undefined, remoteInput: Database | null | undefined): Database {
  ...
  const filterDeleted = <T extends { id: string }>(items: T[]) => items.filter(item => !deleted.has(item.id));
  const merged: Database = {
    ...
    studentLedger: filterDeleted(mergeArrayById(local.studentLedger, remote.studentLedger)),
    payments: filterDeleted(mergeArrayById(local.payments, remote.payments)),
    tutorLedger: filterDeleted(mergeArrayById(local.tutorLedger, remote.tutorLedger)),
    slips: filterDeleted(mergeArrayById(local.slips, remote.slips)),
    kas: filterDeleted(mergeArrayById(local.kas, remote.kas)),
    otherIncomes: filterDeleted(mergeArrayById(local.otherIncomes, remote.otherIncomes)),
    attendanceReports: filterDeleted(mergeAttendance(local.attendanceReports, remote.attendanceReports)),
    ...
    adminPassword: remote.adminPassword || local.adminPassword || "admin123",
    deletedIds,
    lastUpdated: [local.lastUpdated, remote.lastUpdated].filter(Boolean).sort().pop() || new Date().toISOString()
  };
  return recalculateAllLedgers(merged);
}
export function recalculateAllLedgers(input: Database): Database {
  const db = ensureDatabaseDefaults(JSON.parse(JSON.stringify(input)));
  const rebuild = <T extends { tanggal: string; id: string }>(items: T[], apply: (item: T, running: number) => number): T[] => {
    const indexed = items.map((item, index) => ({ item, index }));
    indexed.sort((a, b) => a.item.tanggal.localeCompare(b.item.tanggal) || a.index - b.index);
    let running = 0;
    for (const entry of indexed) { running = apply(entry.item, running); (entry.item as any).saldoBerjalan = running; }
    return indexed.sort((a, b) => a.index - b.index).map(x => x.item);
  };
  db.studentLedger = rebuild(db.studentLedger, (tx, running) => running + (tx.tipe === "debit" ? Number(tx.jumlah) || 0 : -(Number(tx.jumlah) || 0)));
  db.tutorLedger = rebuild(db.tutorLedger, (tx, running) => running + (tx.tipe === "kredit" ? Number(tx.jumlah) || 0 : -(Number(tx.jumlah) || 0)));
  db.kas = rebuild(db.kas, (tx, running) => running + (tx.tipe === "masuk" ? Number(tx.jumlah) || 0 : -(Number(tx.jumlah) || 0)));
  db.lastUpdated = new Date().toISOString();
  return db;
}
export function deleteFromDatabase(db: Database, collection: keyof Database, id: string): Database {
  const next = ensureDatabaseDefaults(db);
  ...
  next.deletedIds = Array.from(new Set([...(next.deletedIds || []), id]));
...
export async function deleteRecordFromSupabase(table: string, id: string): Promise<boolean> {
  try {
    ...
    const allowed: Record<string, string> = { siswa: "siswa", tutor: "tutor", program: "program", jadwal: "jadwal", laporan_kehadiran: "laporan_kehadiran" };
    const target = allowed[table] || table;
    const { error } = await supabase.from(target).delete().eq("id", id);
    if (error) { console.warn(`[Supabase] Gagal menghapus ${target}/${id}:`, error.message); return false; }
    return true;
  } catch (error) { console.warn("[Supabase] Delete helper gagal:", error); return false; }
}
...
export default {
  getDatabase, saveDatabase, updateDatabase, getLocalDatabase, saveLocalDatabase,
  ensureDatabaseDefaults, generateCleanDatabase, clearPrototypeData,
  mergeDatabases, recalculateAllLedgers, generateUniqueId,
  getStudentBalance, getTutorHonorBalance, getTutorDepositBalance, getKasLembagaBalance,
  filterByDateRange, deleteRecordFromSupabase, deleteFromDatabase,
  updateBroadcastMessageTransaction, checkDuplicateSession, addSessionTransaction,
  deleteSessionTransaction, addPaymentTransaction, confirmTutorDepositHandover,
  undoTutorDepositHandover, payTutorHonorTransaction, addGeneralExpenseTransaction,
  addOtherIncomeTransaction, submitAttendanceReport, verifyAttendanceReport,
  undoVerifyAttendanceReport, deleteAttendanceReport, getNamaHariIndo,
  addScheduleTransaction, deleteScheduleTransaction
};